...
ptr<resp_msg> raft_server::handle_vote_req(req_msg& req) {//收到来自其他节点的投票请求
    ptr<resp_msg> resp(cs_new<resp_msg>(state_->get_term(), msg_type::vote_response, id_, req.get_src()));
    bool log_okay = req.get_last_log_term() > log_store_->last_entry()->get_term() ||//投票请求中的最后一次日志所属任期 要大于 自己的最后一次日志所属任期
        (req.get_last_log_term() == log_store_->last_entry()->get_term() &&//或者所属任期一样情况下，
            log_store_->next_slot() - 1 <= req.get_last_log_idx()); //投票请求中的最后一次日志索引不能小于自己的最后一次日志索引
...
ptr<resp_msg> raft_server::handle_prevote_req(req_msg& req) {//收到来自其他节点的预投票请求
    ...
    bool grant = req.get_term() >= state_->get_term() && log_okay; //预投票请求中的任期不能小于自己当前的任期，并且日志也满足条件，才接受这次预投票请求
    ...
    if (grant) { //预投票请求被接受，resp 带上自己的下一次日志索引
        resp->accept(log_store_->next_slot()); 
    }
...
void raft_server::become_candidate() {
    prevote_state_.reset(); //关闭预投票，准备发起正式投票
    state_->inc_term(); //当前任期+1
...
void raft_server::request_vote() {
    ...
    bool change_to_leader(false);
    {
        ...
        if (votes_granted_ > (int32)(peers_.size() + 1) / 2) { //如果整个集群就自己一个节点，直接指定自己为leader即可。
            election_completed_ = true;
...
void raft_server::request_append_entries() {
    read_lock(peers_lock_);
    if (peers_.size() == 0) { //集群中只有自己，直接提交日志
...
void raft_server::handle_prevote_resp(resp_msg& resp) {
    ...
    //所以当prevote_state_为空时，意味着一轮预投票已经完成。
    if (!prevote_state_) { 
...
void raft_server::snapshot_and_compact(ulong committed_idx) {
    if(ctx_->params_->snapshot_distance_ == 0 || (committed_idx - log_store_->start_index()) < (ulong)ctx_->params_->snapshot_distance_) {
